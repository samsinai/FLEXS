...
class Moran(Evolution):
    ...
    def __init__(self, mu=0.01, rho=1, recomb_rate=0.0, beta=100, batch_size = 100, alphabet ="UCGA" , virtual_screen = 0):

        super(WF, self).__init__(mu, rho, recomb_rate, beta, batch_size , alphabet , virtual_screen)
        self.explorer_type =f'Moran_mu{self.mu}_r{self.recomb_rate}_rho{self.rho}_beta{self.beta}'
    ...
    def propose_samples(self):
        ...
        if self.recomb_rate > 0: 
            for i in range(self.rho): 
                recombined_replicated_sequences_half = self.recombine_population(replicated_sequences) [:self.batch_size]
        else:
