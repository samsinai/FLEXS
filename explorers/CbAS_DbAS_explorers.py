        ...
        while (not_converged) and (count < self.batch_size * self.virtual_screen):
            ...
            while len(proposals) == 0:
                try:
                    ...
                    return self.all_proposals_ranked[-self.n_new_proposals:]
                    # print(e.message)
                    # print('Resetting the model (excluding the current batch of proposals)...')
                    # all_samples = all_samples_and_weights[0][:-self.batch_size]
                    # all_weights = all_samples_and_weights[1][:-self.batch_size]
                    # all_samples_and_weights = (all_samples, all_weights)
                    # self.generator.get_model(seq_size=len(initial_batch[0]), alphabet=self.alphabet)
                    # self.generator.train_model(all_samples, all_weights)
