...
class Multi_dimensional_model(Model):
    ...
    def populate_model(self,data=None):
        for sequence in data:
            self.sequences[sequence]=tuple([model.get_fitness(sequence) for model in self.models])

    def _fitness_function(self,sequence):
        #return tuple([model.get_fitness(sequence) for model in self.models])
        return self.combined_func([model.get_fitness(sequence) for model in self.models])
    ...
    def get_fitness(self,sequence):
         if sequence in self.sequences:
            ...
            self.expanded_sequences[sequence]=list([model.get_fitness(sequence) for model in self.models])
            return self.sequences[sequence]

    def break_down_fitness_and_sort(self):
        sorted_by_value = sorted(self.sequences.items(), key=lambda kv: kv[1],reverse=True)
        output=[]
