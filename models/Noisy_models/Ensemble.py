...
class Ensemble_models(Model):
      def __init__(self,list_of_models=None, adaptive=True):
         if list_of_models:
            ...
            self.weighted_r2s = [1./len(list_of_models) for i in range(len(list_of_models))]
      ...
      @property
      def model_type(self):
          return f'{self.name}_{len(self.models)}_'+("_").join(set([model.model_type for model in self.models]))
      ...
      def add_model(self, model):
          ...
          self.r2s = [1/len(self.list_of_models) for i in range(len(self.list_of_models))]
