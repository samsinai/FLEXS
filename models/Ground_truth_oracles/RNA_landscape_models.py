...
class RNA_landscape_constructor():
    ...
    def construct_landscape_object(self,landscape_dict):
        ...
        landscapes = [] 
        if landscape_params["self_fold_max"] != False: #This is currently not used for any landscape, the idea would be to pass a upper bound on self-folding, beyond which the sequence is considered unviable. 
           l = RNA_landscape_folding(threshold = landscape_params["self_fold_max"], reverse = True )
           landscapes.append(l)
        ...
        if len(landscapes)>1:
           ...
           combined_func=lambda z: reduce(lambda x, y: x*y,z) #currently multiplicative landscapes
           composite_landscape = Multi_dimensional_model(landscapes,combined_func=combined_func)

        else:
           return {"landscape_id": landscape_id, "starting_seqs": landscape_params["starts"], "landscape_oracle": landscapes[0]} 


        if landscape_params["conserved_pattern"]:
           composite_landscape = Conserved_RNA_landscape_cont(composite_landscape,landscape_params["conserved_pattern"],landscape_params["conserved_start"])

        return {"landscape_id": landscape_id, "starting_seqs": landscape_params["starts"], "landscape_oracle": composite_landscape} 
